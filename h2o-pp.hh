...
struct H2OWebserver
{
  ...
  explicit H2OWebserver(const string_view hostname="default", int port=65535);
  ...
  h2o_hostconf_t* addHost(const string_view hostname, int port=65535);
  ...
  void addDirectory(const string_view path, const string_view directory, h2o_hostconf_t* hconf=0);

  //! Call this to add a TLS context. Returns an accept context that can be used with addListener
  h2o_accept_ctx_t* addSSLContext(const string_view certificate, const string_view key, const string_view ciphers="");
